...
import pdp from './../img/profil_defaut.png';
      ...
      .then((json) => {
        if (json.message == 'pas de demande a ce nom') {
          ...
          if (json.resultat[0] == undefined) {
            test.push(
              <View style={styles.group} key={1}>
                <View style={styles.rect2}>
                  <Text style={styles.ericCartman}>
                    Vous n'avez pas de demande{'\n'} Veuillez effectuer une demande avant de pouvoir noter un
                    utilisateur{' '}
                  </Text>
                </View>
              </View>,
            ...
            for (let i = 0; i < json.resultat.length; i++) {
              if (
                json.resultat[i].donneurId == this.state.donneurId &&
                json.resultat[i].idDemande == this.state.idDemande
              ) {
                    ...
                    <View style={styles.group} key={i}>
                      ...
                      <View style={styles.rect2}>
                        <View style={styles.ericCartmanStack}>
                          <Text style={styles.ericCartman}>
                            {json.resultat[i].Prenom}
                            {'\n'}
                            {json.resultat[i].Nom}
                          </Text>
                          <Image source={pdp} resizeMode="contain" style={{width: 70, height: 70, marginTop: 10}} />
                        </View>
                        <AirbnbRating
                          count={5}
                          reviews={['décevant', 'moyen', 'bon', 'top', 'Jesus']}
                          defaultRating={json.resultat[i].rating}
                          size={20}
                        ...
                        <TextInput
                          ...
                          value={this.state.commentaire[i]}
                        />{' '}
                        <TouchableOpacity style={styles.containerButton} onPress={() => this.onPressButton()}>
                    ...
                    <View style={styles.group} key={i}>
                      ...
                      <View style={styles.rect2}>
                        <View style={styles.ericCartmanStack}>
                          ...
                          <Image
                            source={{uri: json.resultat[i].PhotoProfil}}
                            resizeMode="contain"
