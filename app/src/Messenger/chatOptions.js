...
function ChatOption(route) {
  ...
  function getMembers() {
    fetch(`https://help-recover-api.herokuapp.com/groupUsers/${route.route.params.id}`)
  ...
  const leaveGroup = (id, bol) => {
    fetch(`https://help-recover-api.herokuapp.com/group/${id}`, {
      method: 'DELETE',
    })
      .then((res) => res.json())
      .then(() => {
  ...
  const deleteGroup = () => {
    fetch(`https://help-recover-api.herokuapp.com/group/all/${route.route.params.id}`, {
      method: 'DELETE',
    })
  ...
  return (
    <View>
      ...
      {ownerId !== userId && <Button onPress={() => leaveGroup(userId, false)} title="QUITTER LE GROUPE" color="red" />}
