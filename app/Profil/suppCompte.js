...
class suppCompte extends React.Component {
    ...
    submit() {
        var valeur = "";
        fetch(`http://192.168.0.15:3000/usersMM/${this.state.mail}`).then(response => response.json())
        .then(json => {
          valeur = json;
          }).catch((error) => {
            console.log(error);
        ...
        if (valeur[0].Mail == this.state.mail && bcrypt.compare(this.state.motdepasse, valeur[0].password)) { //valeur[0].password est à décodé
        ...
        return ( 
            <View>
                <Text style={styles.text}>
                    Afin de supprimer votre compte, veuillez introduire votre email et votre mot de passe. Vous recevrez un email de confirmation à l'adresse mail indiquée. L'email sera valide pendant 1h.
                </Text>
                <TextInput
